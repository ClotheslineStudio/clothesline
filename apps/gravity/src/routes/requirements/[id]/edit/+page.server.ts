...
export const actions = {
	update: async ({ request, params, url }) => {
		...
		if (patch.dueDate !== undefined) {
			...
			if (normalized === undefined) {
				return fail(400, {
					ok: false,
					message: 'Please fix the highlighted fields.',
					fieldErrors: { dueDate: ['Enter a valid due date (YYYY-MM-DD)'] },
					values: raw
				});
			}
		...
		try {
			...
			console.error('[requirements edit update action] unexpected error', err);
			return fail(500, { ok: false, message: 'Unable to save changes. Please try again.' });
		}

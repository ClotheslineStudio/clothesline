		...
		<section class="mt-6 rounded-2xl border border-white/10 bg-white/5 p-4">
			...
			{#if status === 'loading'}
				<div class="space-y-3">
					{#each Array(6) as _}
						<div
							class="grid grid-cols-[auto,1fr,auto] items-center gap-3 rounded-xl border border-white/10 bg-white/5 px-4 py-3"
						>
			...
			{:else if sources.length === 0}
				...
				<div
					class="rounded-2xl border border-dashed border-white/15 bg-zinc-950/20 p-8 text-center"
				>
					...
					<div class="mt-4 flex flex-wrap justify-center gap-2">
						<a
							class="rounded-xl bg-white px-4 py-2 text-sm font-medium text-zinc-950"
							href="/_dev/source/url?workspaceId={workspaceId}"
						>
							Add URL Source
						</a>
						<a
			...
			{:else if visible.length === 0}
				...
				>
					...
					<p class="mt-1 text-sm text-zinc-400">
						Try clearing your search or changing the type filter.
					</p>

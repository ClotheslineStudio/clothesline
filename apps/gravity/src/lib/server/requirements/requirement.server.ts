...
export async function createRequirement(
	...
	if (v.dueDate && dueDate === undefined) {
		return {
			...
			error: validationError('Please fix the highlighted fields.', {
				dueDate: 'Enter a valid due date (YYYY-MM-DD)'
			})
		};
	}
	...
	try {
		...
		console.error('[createRequirement] failed to create requirement', error);
		return { ok: false, error: validationError('Unable to create requirement. Please try again.') };
	}
...
export async function updateRequirement(
	...
	if (Object.keys(data).length === 0) {
		return {
			ok: false,
			error: validationError('Validation failed.', { form: 'No fields provided to update.' })
		};
	}
	...
	try {
		result = await prisma.requirement.updateMany({
			where: { id: v.id, workspaceId, archivedAt: null },
			data
		});
	} catch (error) {
		console.error('[updateRequirement] failed to update requirement', error);

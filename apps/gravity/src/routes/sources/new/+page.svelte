<script lang="ts">
  ...
  async function submitUrl() {
    ...
    try {
      ...
      await goto(`/sources?workspaceId=${encodeURIComponent(workspaceId)}&created=${encodeURIComponent(created.id)}`);
    } catch (e) {
  ...
  async function uploadPdf() {
    ...
    if (file.size > MAX_SOURCE_FILE_BYTES) {
      error = `File too large. Max ${formatBytes(MAX_SOURCE_FILE_BYTES)}.`;
      return;
    }
...
<div class="mx-auto max-w-3xl px-6 py-8">
  <div class="mb-6">
    ...
    <p class="mt-1 text-sm text-zinc-600">Upload a PDF or add a URL. This creates a Source record in the workspace.</p>
  </div>
    ...
    <div class="rounded-xl border border-zinc-200 bg-white p-6">
      ...
      <div class="mt-4 grid gap-4">
        <label class="grid gap-1">
          ...
          <input class="rounded-lg border border-zinc-200 px-3 py-2 text-sm" bind:value={pdfTitle} placeholder="If blank, filename is used" />
        </label>
    ...
    <div class="rounded-xl border border-zinc-200 bg-white p-6">
      ...
      <div class="mt-4 grid gap-4">
        <label class="grid gap-1">
          ...
          <input class="rounded-lg border border-zinc-200 px-3 py-2 text-sm" bind:value={title} placeholder="Ex: IDEA Part B Guidance (2026)" />
        </label>

        <label class="grid gap-1">
          <span class="text-sm text-zinc-700">URL (required)</span>
          <input class="rounded-lg border border-zinc-200 px-3 py-2 text-sm" bind:value={url} placeholder="https://…" />
        </label>

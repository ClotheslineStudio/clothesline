<script lang="ts">
	...
	function validateClient() {
		...
		if (dueDate && !isValidYyyyMmDd(dueDate)) fe.dueDate = 'Enter a valid due date (YYYY-MM-DD)';
		fieldErrors = fe;
		formError = null;
		...
		<section class="card">
			...
			<div class="grid">
				...
				<label class="field span2">
					...
					<textarea bind:value={body} rows="6" placeholder="Optional description / notes…"
					></textarea>
					{#if fieldErrors.body}
			...
			<div class="footer">
				<div class="subtle">Title is required. Status/Priority default if not set.</div>
			</div>
		</section>
	</div>
</div>

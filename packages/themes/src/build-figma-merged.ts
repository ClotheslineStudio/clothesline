...
//  • $themes mapping so Figma Variables auto-create modes
//    (Light/Dark + Vision + Contrast)
// ===============================================================
...
// ===============================================================
// Build the $themes structure
// ===============================================================
const VISION_MODES = ["Protanopia", "Deuteranopia", "Tritanopia", "Monochrome"];
const CONTRASTS = ["Low", "Default", "High"];

function buildThemesBlock() {
  return THEMES.map((t) => {
    const group = t.name.replace(/(^|\s)\S/g, (s) => s.toUpperCase());
    const modes: any[] = [];

    for (const baseMode of ["Light", "Dark"] as const) {
      // Base mode
      modes.push({
        name: `${baseMode}`,
        selectedTokenSets: { global: "enabled", [`${t.name}.${baseMode.toLowerCase()}`]: "enabled" },
      });

      // Add vision × contrast combinations
      for (const contrast of CONTRASTS) {
        for (const vision of VISION_MODES) {
          modes.push({
            name: `${baseMode} – ${contrast} – ${vision}`,
            selectedTokenSets: { global: "enabled", [`${t.name}.${baseMode.toLowerCase()}`]: "enabled" },
          });
        }
      }
...
async function run() {
  ...
  const file: any = {
    $description: "Clothesline tokens (merged) for Tokens Studio (with Vision + Contrast modes)",
    $metadata: {
